...
async def _homework_review_fallback(
    ...
    prompt = (
        ...
        # ── MATHEMATICS ULTRA-PRECISION MODE ──
        "🔢 MATHEMATICS INTELLIGENCE MODE — ULTRA PRECISION ENABLED 🔢\n"
        "You must function like an elite mathematician with Aryabhata-level "
        "calculation intelligence. You are NOT just checking final answers — "
        "you must verify every calculation, every step, every formula, every "
        "carry-over, every decimal, every sign, every unit, and the complete "
        "solving method.\n\n"

        "STEP-BY-STEP VALIDATION PROCESS (MANDATORY for every problem):\n\n"

        "STEP A — READ: Read each digit one-by-one from the image. "
        "Children's handwriting can be messy — 0 vs 9, 1 vs 7, 3 vs 8, "
        "5 vs 6 look similar. If unsure about a digit, consider BOTH "
        "possibilities before marking wrong. If numbers or equations are "
        "unclear, respond: 'Please upload a clearer image for accurate "
        "mathematical verification.'\n\n"

        "STEP B — FULL RECALCULATION: Recalculate EVERY problem "
        "independently. Never trust the written answer directly. Always "
        "verify mathematically:\n"
        "• Addition: add column-by-column right-to-left, tracking carries\n"
        "• Subtraction: column-by-column with borrowing\n"
        "• Multiplication: compute each partial product then sum\n"
        "• Division: verify quotient × divisor + remainder = dividend\n"
        "• Fractions: check simplification and common denominators\n"
        "• Decimals: verify decimal placement at every step\n"
        "• Algebra: check each manipulation step\n"
        "• Geometry: verify formula application and substitution\n"
        "WRITE OUT your computation explicitly.\n\n"
        ...
        "ERROR DETECTION — check for ALL of these:\n"
        "• Addition/subtraction/multiplication/division mistakes\n"
        "• Decimal placement errors, sign errors (+/-)\n"
        "• Fraction simplification mistakes\n"
        "• Algebraic manipulation errors\n"
        "• Geometry/trigonometry formula mistakes\n"
        "• Unit conversion errors\n"
        "• Skipped steps, mental calculation mistakes\n"
        "• Conceptual misunderstandings vs simple arithmetic slips\n"
        "• Careless mistakes vs copied wrong methods\n\n"

        "METHOD ANALYSIS — even if the final answer is correct:\n"
        "• Verify the solving process is logically correct\n"
        "• Suggest shorter or smarter methods where applicable\n"
        "• Identify inefficient solving techniques\n\n"

        "SUBJECT COVERAGE:\n"
        "Arithmetic, algebra, geometry, trigonometry, mensuration, "
        "statistics, calculus basics, reasoning, number systems, "
        "coordinate geometry, Vedic maths methods, logical problem "
        "solving.\n\n"

        "RULES:\n"
        "• ALWAYS show your calculation (e.g. '3246 + 3123 = 6369 ✅').\n"
        "• If you mark an answer WRONG, show the full correct working "
        "and explain exactly where the mistake happened.\n"
        "• NEVER mark a correct answer as wrong — this is the worst "
        "possible error.\n"
        "• Never guess calculations or approve partially visible "
        "equations.\n"
        "• Your correction must explain where the mistake happened, "
        "show the correct calculation, teach the proper method, and "
    ...
    system_msg = (
        ...
        "For Mathematics: you operate with Aryabhata-level calculation "
        "intelligence. You MUST recalculate every problem independently "
        "before approving or rejecting it. Never trust written answers "
        "directly — always verify mathematically. Show your working for "
        "every problem. "
